...
try {
    ...
    if(!file_exists(CACHE_DBFILENAME)) {
        ...
        $create_query = file_get_contents('./sqlite_template.sql');
    ...
    function db_getFaculty($faculty) {
        global $pdo;
        ...
        if (empty($data) || (isset($data['date']) && computeOldDate($data['date']) > CACHE_TIMELEFT)) {
            ...
            $cache = icress_getFaculty($faculty);

            if(empty($data))
                $sth = $pdo->prepare("INSERT INTO data
                                      VALUES (?, ?, datetime('now'))")->execute(array($faculty, $cache));

            else if (computeOldDate($data['date']) > CACHE_TIMELEFT)
                $sth = $pdo->prepare("UPDATE data
                                      set data = ?, date = datetime('now')
                                      WHERE name = ?")->execute(array($cache, $faculty));
    ...
    function db_getSubject($faculty, $subject) {
        global $pdo;
        ...
        if (empty($data) || (isset($data['date']) && computeOldDate($data['date']) > CACHE_TIMELEFT)) {
            ...
            $cache = icress_getSubject($faculty, $subject);
